<template>
  ...
  <div class="container">
    <div class="wrapper">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" hidden style="display: none;" xmlns:xlink="http://www.w3.org/1999/xlink">
        <defs>
...
export default {
  ...
  methods: {
    getMusics() {
      let vm = this;
      axios.get('https://api.imjad.cn/cloudmusic/?type=playlist&id='+this.playlistID)
      .then((res) => {
        // console.log(res)
        ...
        for (var i=0; i<pl.length; i++) {
          ...
          let newTrack = {
            name: pl[i].name,
            artist: pl[i].ar[0].name,
            cover: pl[i].al.picUrl,
            source: '',
            url: '',
            favorited: false
          };
          axios.get('https://api.imjad.cn/cloudmusic/?type=song&id='+pl[i].id)
          .then((srs) => {
            // console.log(srs);
