...
实际上就是给某些后面可能经常用到的正则表达式取一个别名，从而简化词法规则的书写。定义部分的格式一般为：
...
这部分代码会被原封不动地拷贝到`lex.yy.c`中，以方便用户自定义所需要执行的函数，main函数也可以写在这里。

### 解题思路



### Level1：一个简单的 lex 代码如下：
